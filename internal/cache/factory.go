...
type CacheLimits struct {
	UAClosureMaxEntries     int `yaml:"ua_closure_max_entries" json:"ua_closure_max_entries"`           // Max entries in subject->UA closure cache (default: 50k)
	...
	AllowCacheMaxSubjects int `yaml:"allow_cache_max_subjects" json:"allow_cache_max_subjects"` // Max number of subject keys in allow cache (default: 10k)
	AllowCacheMaxEntries  int `yaml:"allow_cache_max_entries" json:"allow_cache_max_entries"`   // Max total (subject,op) pairs in allow cache (default: 100k)
	DenyCacheMaxSubjects  int `yaml:"deny_cache_max_subjects" json:"deny_cache_max_subjects"`   // Max number of subject keys in deny cache (default: 10k)
	DenyCacheMaxEntries   int `yaml:"deny_cache_max_entries" json:"deny_cache_max_entries"`     // Max total (subject,op) pairs in deny cache (default: 100k)
...
type Caches struct {
	...
	AllowCache    *SubjectOpBitmapCache
	DenyCache     *SubjectOpBitmapCache
...
func NewCaches(cacheTTL time.Duration, limits CacheLimits) *Caches {
	...
	if limits.AllowCacheMaxSubjects <= 0 {
		limits.AllowCacheMaxSubjects = 10000
	}
	if limits.AllowCacheMaxEntries <= 0 {
		limits.AllowCacheMaxEntries = 100000
	}
	if limits.DenyCacheMaxSubjects <= 0 {
		limits.DenyCacheMaxSubjects = 10000
	}
	...
	return &Caches{
		...
		AllowCache:    NewSubjectOpBitmapCache(cacheTTL, limits.AllowCacheMaxSubjects, limits.AllowCacheMaxEntries),
		DenyCache:     NewSubjectOpBitmapCache(cacheTTL, limits.DenyCacheMaxSubjects, limits.DenyCacheMaxEntries),
